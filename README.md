...
- **Messages**: every piece of UI text in *either* GUI app — toasts, tooltips, labels, empty states — lives in a per-app `messages.json`, compiled into that app's frontend as the shipped defaults but also written out at runtime next to its executable (`script-manager-gui.messages.json` / `sm-config-edit.messages.json`), self-seeded from those defaults the first time that app runs. A tab per app (**script-manager-gui** / **sm-config-edit (this app)**) picks which one you're editing; each key's row is grouped under a collapsible category heading (click the category to expand/collapse it — matches the ▸/▾ convention used elsewhere). If `script-manager-gui` has never been run yet, its file doesn't exist and editing its messages here shows a message asking you to run it once first. Two icon buttons sit at the top right: **Restore defaults** resets every field on screen back to that app's shipped text (a separate read-only snapshot, refreshed on every launch, next to the same executable) without saving — you still need **Save** to persist it, same as any other edit here; **Save** writes the whole file back and flashes a reminder that the change only takes effect the next time the edited app is launched — there's no live-reload while it's already running, the same as a config edit needing Refresh/F5 in the other app.
