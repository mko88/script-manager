...
`cmd` and `script` are mutually exclusive per action — set one or the other, not both. `cmd` is a shell-template command string, expanded and run through the configured `shell:`. `script` is a file path (also `{{.field}}`-template-expanded) run **directly** with no shell wrapping and no extra flags — the file is expected to already know how to run itself (a shebang line on Linux, or a natively runnable file — `.exe`/`.bat`/`.cmd` — on Windows; something like a bare `.ps1` needs its own shebang-equivalent or should be run via `cmd:` with `pwsh -File` instead). Either way the action still gets the same environment variables (the item's fields plus the global `env:` block, uppercased) that a `cmd:` action gets.
