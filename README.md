...
- Actions can be filtered by group with a row of chips below the Actions list header ("All" + one per group found on the item's actions), same grouping the TUI cycles through with `[` / `]`. Unlike the TUI, multiple groups can be selected at once — an action must belong to *all* selected groups to show (AND, not OR); clicking "All" clears the selection back to everything. Each chip shows how many actions would match if it were added to the current filter, e.g. `diagnostics(5)`. Chips are always sorted by that count (default: descending) with name as the tie-breaker (default: A-Z); the `#` and `A-Z` buttons above the chips each flip their own key's direction — they combine rather than replace each other. The chip row itself is collapsible (▾/▸) — collapsed, it shows the selected groups as text (e.g. "Groups: safe, diagnostics"); this collapsed state persists across restarts like the other panes
