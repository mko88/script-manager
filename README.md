...
`F5` works in either mode. It re-reads the config file, refreshes items/actions/details in place, and preserves your current selection where still valid. If the file fails to read or has a YAML syntax error, the previous config is kept and the status bar shows the error instead of wiping your session.
...
`cmd` and `script` are mutually exclusive per action — set one or the other, not both. `cmd` is a shell-template command string, expanded and passed to the configured `shell:` as a single `-c`/`-Command` argument. `script` is a file path (also `{{.field}}`-template-expanded) that the configured `shell:` invokes directly instead — no template expansion of the file's own contents, and none of `cmd:`'s extra command-string flags — but still through the shell's own native invocation, not run as source. That's what lets one `script:` action correctly handle both a plain interpreted script (a bare `.ps1` works on Windows even though it isn't natively executable — no need to route it through `cmd:` with `pwsh -File` instead — and a POSIX script keeps using its own shebang line for whichever interpreter it names, even one other than `shell:` itself) and an already-native executable (`.exe`/`.bat`/`.cmd` on Windows, a POSIX binary) without needing to know in advance which kind it is. Either way the action still gets the same environment variables (the item's fields plus the global `env:` block, uppercased) that a `cmd:` action gets.
...
- Command preview (expanded template) for the selected action, with a copy button. The action's groups are shown as chips between the description and the command, colored the same way as the Actions group filter chips. The pane's content is split into collapsible sections (▾/▸ headers, same style as the config editor's grouped fields; the header titles have their own `section-title` theme token, separate from the general highlight color): **COMMAND** holds the description, group chips, and the command/script source; **OUTPUT** sits above it and appears once a "Run here" run has started. The OUTPUT header's right side shows the run's status: "Running…" with a pulsing dot while the command runs, then "Exit code: X" with a dot that's green for exit code 0 and red for anything else (all three dot colors are theme tokens — `run-active` for the pulsing running dot, `run-ok`/`run-error` for the exit-code ones — customizable in the config editor's Theme section like any other color). Each section's collapsed state persists across restarts, independently of the pane's own collapse
...
The **Run** button in the Command pane opens the expanded command in a terminal window. For a `script:` action, the Command pane shows the path (`Script: <path>`) plus the file's own source underneath, line-numbered the same way an expanded `cmd:` command is; **Run**/**Run here** invoke that file directly through the configured `shell:` (e.g. `pwsh -File`/a bare POSIX invocation) rather than expanding a `cmd:` template — same env vars, same buttons either way. By default it auto-detects the most common terminal for the current OS, tried in this order until one is found on `PATH`:
...
- The starting directory is the app-data directory (`%AppData%\script-manager` on Windows, `~/.config/script-manager` on Linux — not the user's home directory or the GUI executable's own folder), so a relative path in a `cmd:` template — or any file the script itself writes with a relative path — lands there, a location that's reliably user-writable even when the binaries are installed somewhere read-only
...
For a command that isn't expected to need interactive input, the **Run here** button next to **Run** executes it directly and streams the captured output — stdout and stderr interleaved in real execution order — live into the Command pane's **OUTPUT** section as it's produced, instead of opening a terminal window. A **Cancel** button appears while it's running, and forcibly terminates the whole process tree (not just the top-level shell); besides the pulsing dot next to the running item/action, the OUTPUT section header shows "Running…" while it runs and the exit code once it finishes, and **Copy output** copies whatever's been captured so far to the clipboard — even while the command is still running. Like **Run**, the working directory is the app-data directory, and stdin is left disconnected — a command that unexpectedly prompts for input fails fast with an immediate EOF rather than hanging with no terminal for anyone to type into.
...
Different actions can run inline at the same time — starting another action while one is still going doesn't stop it, and a small pulsing dot marks which items and actions currently have a run in progress, so you can tell at a glance without leaving the one you're looking at. Once a run finishes, the action's row keeps a steady dot showing its last result — green for exit code 0, red otherwise (the same `run-ok`/`run-error` theme tokens as the OUTPUT header's dot), with the exact code in the tooltip — until the app closes or that action is run again (while re-running, the pulsing dot takes over). Switching back to an action that's still running (or has since finished) picks its output back up right where it left off — only starting the same action again while it's already running is rejected.

#### Toolbar

Three controls above the panes: **Load config** browses for a different YAML file and switches to it (including redirecting **Refresh config**/F5 to reload *that* file from then on); **Refresh config** re-reads the current file, same as pressing F5; a gearbox icon at the far right is **Open config editor** (also **Ctrl+E**), which launches `sm-config-edit` — the [Config Editor](#config-editor) below — pointed at whichever config file is currently loaded; clicking it again while that instance is still running is a no-op (with a toast saying so) rather than opening a second window.

#### Theme

Both GUI apps default to the dark theme. The active theme is applied before the UI renders on every launch, so there's no flash of the wrong theme. It's shared between the two apps: each keeps its own `localStorage` copy for an instant first paint, but every change also persists to a small `sm-theme.json` file in the app-data directory (`%AppData%\script-manager` on Windows, `~/.config/script-manager` on Linux), and every startup reconciles against it — so switching, saving, renaming, or deleting a theme in one app carries over the next time you open the other.

script-manager-gui goes further and watches `sm-theme.json` for changes while it's running, so a theme switched, saved, or deleted in the Config Editor shows up there **live** — no relaunch needed, unlike a config edit which still needs Refresh/F5. sm-config-edit doesn't watch it back (its own changes already apply to itself immediately).

Neither app's toolbar has a theme switcher — switching themes, and creating, renaming, or deleting custom ones, all happen from the Config Editor's own **Theme** section (see below); script-manager-gui only ever reflects whatever's currently active.
...
- **New / Open / Save / Save As** (Ctrl+N/O/S/Shift+S), same auto-detect rule as the TUI/GUI: on launch it resolves `config-win.yaml`/`config.yaml` (exe dir then working directory) the same way `-config`-less TUI/GUI startup does, with a native file picker to open a different file or choose where to save a new one. Finding nothing on launch isn't treated as an error — it just starts blank, since a first-time user of this tool plausibly has no config yet. Ctrl+S also saves the Theme section's working theme, if the Theme section is open and there's a non-empty name to save under (see Themes below) — one shortcut for both, since a theme isn't part of `config.yaml` itself. The toolbar is a flush header bar above the rest of the window, the same treatment `script-manager-gui`'s toolbar gets. Two icon buttons are pinned to its far right, sharing the same Go-side helper (`cmd /c start` on Windows, `xdg-open` on Linux) rather than Wails' `BrowserOpenURL`, which validates its argument as an http(s) URL and refuses local file paths outright: a folder-icon **Open data folder** button — disabled only if the OS-standard per-user config location couldn't be resolved, tooltip "Open data folder: *path*" — opens the app-data directory (`%AppData%\script-manager` on Windows, `~/.config/script-manager` on Linux — see [Configuration](#configuration) above) in the OS's default file manager; a pencil-icon **Open in default editor** button next to it — disabled until something's loaded/saved, tooltip "Open in default editor: *path*" — opens the current config file with the OS's default handler for its file type.
- **Sections**: Items, Action Groups, Actions, Displays, Environment, Shell, Terminal, Theme, Messages — one form per top-level `config.yaml` concern (Theme and Messages aren't part of `config.yaml` itself — see below), listed in their own bordered panel on the left (the same **panel** treatment `script-manager-gui` gives its Items/Actions lists) next to the panel for whichever section is selected. Action Groups edits the `id`/`title`/`color` catalog entries (with a color picker and a swatch next to each entry in the list); an item's reserved keys (`name`, `display`, `actions`, `actionGroups`, `customActions`) get dedicated widgets (text field, a dropdown of configured displays, checkbox lists against the global actions/groups, a repeatable nested action form); anything else is a per-item "Environment" grid (overriding a same-named value from the top-level Environment section), where each value's kind drives which widget edits it: string, multiline (a plain textarea, auto-picked for any existing value with embedded newlines), number, bool, or a raw YAML snippet (auto-picked for anything more complex, like a nested list or map). A lock button next to each field's remove (✕) button marks it secret — independent of kind, so even a multi-line value can be masked — auto-picked when the key ends in "Secret", "Password", or "Key" (case-insensitive, checked live as you type a new field's key too). Removing a field (here or in the top-level Environment section), a Shell command entry, or an item's custom action all ask for confirmation first, same as removing an Item/Action/Action Group/Display already did. A secret field shows a fixed placeholder at rest and only reveals its real value (still masked character-for-character while you edit) once focused, so its length isn't given away by how many dots are shown. Kind and secret are both display hints only — the value saves as the same plain string/number/bool either way, and neither is persisted as such; both are re-derived from the value's shape and the key's name every time the field loads. Every action form (global Actions, and an item's custom actions — same shared form) has a **Command** / **Script file** radio picking which of `cmd`/`script` the action uses; switching it clears whichever field you're leaving, so a stale, unused value never lingers in the saved YAML. Script file mode is a text field plus a **Browse…** button (a native, unfiltered file picker — script files often have no extension at all) or just type a path by hand. Below it, a line-numbered preview of the file's actual source shows automatically (debounced 250ms after typing stops, same as the other live previews) — the same `ScriptSource` component script-manager-gui's Command pane uses for both a `script:` file's source and an expanded `cmd:` command, so both apps show code the same way. The path is used exactly as entered, with no `{{.field}}` template expansion, since a global action isn't tied to any one item to expand against; a templated path just shows a "no such file" error like any other one that doesn't resolve. An action with both `cmd` and `script` set (e.g. from hand-edited YAML) is flagged as a blocking validation error.
...
- **Themes** *(Theme section)*: a collapsible panel at the top picks which theme you're editing. A top-left icon-button toolbar — **Add** (starts a fresh draft seeded from Dark, its name field focused so you can rename it before saving), **Copy** (duplicates whatever's currently loaded — built-in, saved custom, or an in-progress draft — into a new "*name* - copy" draft, matching Displays' own Copy display), and **Delete** (asks for confirmation) — sits above the **Theme**/**Name** row, matching Items/Action Groups/Actions/Displays' own toolbar; there's no in-panel Save button — **Ctrl+S** saves the working theme (whenever there's a non-empty name to save under) the same keystroke that saves `config.yaml` already does, so one shortcut covers both; a top-right group holds **Reset** (reverts an existing custom theme's working colors back to its last-saved state), **Reset to Dark**, and **Reset to Light** (reseeding the working copy from either built-in theme, read live off the stylesheet so they can't drift from it) — all three ask for confirmation first, since they discard whatever's currently unsaved. Picking an existing theme from the **Theme** dropdown, built-in or custom, applies it immediately, everywhere — the same as the old toolbar dropdown did; an in-progress draft shows up in the dropdown too, under its current typed name, so switching away and back doesn't lose it — there's no separate "+ New theme" entry anymore, that's what the Add button is for now. The **Name** field is editable only for a custom theme (Dark/Light show their name read-only). Add and Copy are always available; Delete and Reset are further limited to an existing saved custom theme (disabled for a draft, which has no saved state yet, or for Dark/Light, whose fields aren't editable to begin with); Reset to Dark/Light apply to any custom theme, saved or draft. Below it, every color the app's UI uses (29 CSS custom properties — backgrounds, text, hover/overlay effects) is editable, grouped into collapsible categories (Backgrounds / Text / Effects) the same way Messages groups by category — the color fields themselves are disabled while Dark or Light is selected, since there's nothing to save over a built-in. Each row is a color picker plus a free-text field, so you can type any CSS color, not just what the native picker supports. A live preview panel next to the fields — the page background itself, panel title, list rows (including a selected one; both rows carry all three run-status dots at their right edge, the way script-manager-gui's Items/Actions rows show one, so the dot colors can be judged against both row backgrounds), chips, buttons, an active messages-editor-style tab (previewing `text-tab`), one row per text style (Heading, Normal, Highlighted, Masked, Warning, Error, in that order — each its own clickable example instead of one combined paragraph, so every text-related token is unambiguous to find; Highlighted also hovers like a real copyable value would), an empty collapsible-section header (styled like script-manager-gui's COMMAND/OUTPUT sections, so it previews `section-title`) showing all three run-status dots at once — pulsing running (`run-active`), green exit-0 (`run-ok`), red non-zero (`run-error`) — since a real header only ever shows one at a time, a command block and a command-output block each with its own copy button floated in the corner (matching where script-manager-gui's real copy buttons sit), and, side by side, a toast and a fixed-width box with oversized text that forces a real horizontal scrollbar into view — re-renders on every keystroke using the app's real component styling, so what you see there is exactly what saving will look like. The corner copy buttons are a stand-in for styling that lives in script-manager-gui's own stylesheet (`.cmd-copy-btn`) — sm-config-edit can't reach that app's separate stylesheet, so this preview piece reproduces the same rule locally just so its token (`overlay-soft`) stays reachable from here. The scrollbar box is a stand-in for a different reason: `::-webkit-scrollbar-thumb` is a pseudo-element, not a real DOM node, so nothing can click it or match it by CSS selector no matter which app the rule lives in — clicking the label text inside sets the filter directly instead of going through the usual CSS-matching mechanism. Clicking any preview element — including the blank panel surface itself, which reveals `bg-alt`/`border` — fills a filter box above the field list with the token(s) any CSS rule matching that element (in its resting or hover state) actually references — computed live off the stylesheets, not a hand-maintained list, so a new preview element's own CSS just works here without extra wiring — narrowing the field list to just those. An unselected list row has its own subtle `row-bg` (a shade between the panel surface and the secondary-hover color), so it's a distinct token from the panel it sits on rather than just showing the panel through a transparent background. Buttons and chips share a single `bg-secondary` resting background too — they previously used unrelated tokens (`hover` and `bg-deep`, with separate `hover-strong`/`tint-hover` hover shades) — and both now brighten to the same `secondary-hover` token on hover, matching the secondary-hover color list rows already use. `accent` and `code` were renamed to `text-heading`/`text-highlight` and moved into the Text category — they were always text colors (markdown headings and inline-code/highlighted spans) despite the old "Accents" grouping; a few other, genuinely non-text consumers (focus rings, the checkbox/radio tint, the collapse-toggle glyph) keep using `text-heading` under its new name — the running-command pulse, which used to be on that list too, has since moved to its own `run-active` token alongside `run-ok`/`run-error` since renaming a token doesn't change what it paints, just what it's called. `accent-warm`/`accent-warm-text` went through the same move, renamed to `bg-primary`/`text-primary` into Backgrounds/Text — `bg-primary` is the same warm accent color as before, just named for the role most of its consumers actually use it in (a background); `masked`/`warning`/`error`/`line-number` moved into Text alongside them, since all four are text colors too. That empties the old Accents category out entirely — Backgrounds/Text/Effects covers every token now. `button-bg`/`hover`/`btn-primary-hover` were renamed to `bg-secondary`/`secondary-hover`/`primary-hover` for the same reason: naming a token for the role it plays (primary/secondary) instead of an incidental detail of how it first got used, so a future token filling the same role doesn't inherit a stale name. It can be a few tokens wider than the minimum needed (e.g. clicking the selected row also lists `text`/`secondary-hover`, even though `.row.selected`'s own higher-specificity rule is what actually wins for those properties), but it's still the fastest way to find "which field changes this" without scanning all 29. Saving persists the palette under its name (shared with script-manager-gui the same way the Dark/Light choice is, see [Theme](#theme) above) and makes it active immediately; script-manager-gui picks it up live if it's already running (see [Theme](#theme) above), or on its next launch otherwise.
- **Messages**: every piece of UI text in *either* GUI app — toasts, tooltips, labels, empty states — lives in a shared `internal/messages` Go package (`gui.json` / `configedit.json`) that both binaries embed directly, so either app has both apps' shipped text compiled in regardless of which one you've actually run. Each app also writes its own runtime override file in the app-data directory (`%AppData%\script-manager` on Windows, `~/.config/script-manager` on Linux — `script-manager-gui.messages.json` / `sm-config-edit.messages.json`), seeded from the shipped defaults the first time that app runs; on every later startup, the override is reconciled against the current shipped defaults — a message key the app no longer has is dropped, and a key a newer version added is backfilled — so upgrading never leaves a stale or incomplete override behind. A read-only `*.messages.defaults.json` snapshot of both apps' shipped text is also refreshed in the app-data directory on every launch, regardless of which app is running, for manual reference.

  A tab per app (**script-manager-gui** / **sm-config-edit (this app)**) picks which one you're editing; a search box above the rows filters by key or text (a match inside a collapsed category still shows while searching); each category is a collapsible heading (click it, or use the expand/collapse-all icon button, to toggle — matches the ▸/▾ convention used elsewhere). If `script-manager-gui` has never been run yet, its *override* file doesn't exist yet either — this section still shows its shipped default text (this binary has script-manager-gui's compiled defaults too, the same ones **Restore defaults** always reads from), so there's nothing to run first before you can start editing. Icon buttons at the top right: **expand/collapse all**; **Restore defaults** resets every field on screen back to that app's shipped text (after a confirmation, since it discards whatever's currently in the form) without saving — you still need **Save** to persist it; **Save** writes the whole file back and flashes a reminder that the change only takes effect the next time the edited app is launched — there's no live-reload while it's already running, the same as a config edit needing Refresh/F5 in the other app.
