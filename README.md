...
- **Custom themes** *(Theme section)*: every color the app's UI uses (22 CSS custom properties — backgrounds, text, accents, hover/overlay effects) is editable here, grouped into collapsible categories (Backgrounds / Text / Accents / Effects) the same way Messages groups by category. Each row is a color picker plus a free-text field, so you can type any CSS color, not just what the native picker supports. **Reset to Dark**/**Reset to Light** reseed the whole working copy from either built-in theme (read live off the stylesheet, so they can't drift from it) without touching what's actually applied yet. A live preview panel next to the fields — the page background itself, panel title, list rows (including a selected one), chips, buttons, rendered example markdown (heading/bold/italic/highlight), a command block, a command-output block, error/masked text, and a toast — re-renders on every keystroke using the app's real component styling, so what you see there is exactly what saving will look like. Clicking any preview element fills a filter box above the field list with exactly the token(s) that element's own styling uses (comma-separated, e.g. clicking the selected row fills in `accent-warm, bg`), narrowing the field list to just those — the fastest way to find "which field changes this" without scanning all 22. **Save** persists the palette (shared with script-manager-gui the same way the Dark/Light choice is, see [Theme](#theme) above) and switches this window to it immediately; script-manager-gui picks it up live if it's already running (see [Theme](#theme) above), or on its next launch otherwise, at which point its Theme dropdown also gets a **Custom** option.
