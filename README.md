...
Both GUI apps default to the dark theme. The active theme is applied before the UI renders on every launch, so there's no flash of the wrong theme. It's shared between the two apps: each keeps its own `localStorage` copy for an instant first paint, but every change also persists to a small `sm-theme.json` file next to the executables, and every startup reconciles against it — so switching, saving, renaming, or deleting a theme in one app carries over the next time you open the other.
