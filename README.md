...
- **New / Open / Save / Save As** (Ctrl+N/O/S/Shift+S), same auto-detect rule as the TUI/GUI: on launch it resolves `config-win.yaml`/`config.yaml` (exe dir then working directory) the same way `-config`-less TUI/GUI startup does, with a native file picker to open a different file or choose where to save a new one. Finding nothing on launch isn't treated as an error — it just starts blank, since a first-time user of this tool plausibly has no config yet. Ctrl+S also saves the Theme section's working theme, if the Theme section is open and there's a non-empty name to save under (see Themes below) — one shortcut for both, since a theme isn't part of `config.yaml` itself. The toolbar is a flush header bar above the rest of the window, the same treatment `script-manager-gui`'s toolbar gets. A pencil-icon button pinned to its far right — disabled until something's loaded/saved, tooltip "Open in default editor: *path*" — opens the current config file with the OS's default handler for its file type, via a small Go-side helper (`cmd /c start` on Windows, `xdg-open` on Linux) rather than Wails' `BrowserOpenURL`, which validates its argument as an http(s) URL and refuses local file paths outright.
